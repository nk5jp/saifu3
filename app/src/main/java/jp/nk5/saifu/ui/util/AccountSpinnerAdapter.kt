...
/**
 * 別にSpinnerアカウントでも良いが、学びのために敢えてBaseAdapterを使用する
 */
class AccountSpinnerAdapter(val accounts: List<Account>): BaseAdapter() {
    ...
    /**
     * 指定した行の口座のインスタンスを返却する
     */
    override fun getItem(position: Int) = accounts[position]

    /**
     * 指定した行の口座のidを返却する
     * accountsのidはIntなので、Longに変換する必要がある
     */
    override fun getItemId(position: Int) = accounts[position].id.toLong()
